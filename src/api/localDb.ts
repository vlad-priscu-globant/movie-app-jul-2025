...
export async function loginUser(
  ...
  const data = await httpWrapper<{ token: string }>(
    `${BASE_URL}/login`,
    {
      method: 'POST',
...
export async function fetchPopularMovies(): Promise<Movie[]> {
  const data = await httpWrapper<{ results?: TmdbRawMovie[] }>(
...
export async function fetchMovieDetail(
  ...
  const data = await httpWrapper<TmdbRawMovie>(
...
export async function checkIsFavorite(
  ...
  const data = await httpWrapper<{ isFavorite: boolean }>(
